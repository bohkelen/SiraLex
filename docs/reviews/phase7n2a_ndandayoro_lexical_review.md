# Phase 7N2A Lexical Review — `ndándayoro`
...
This sheet records owner linguistic approval for the 7N2A candidate `ndándayoro`. It does not authorize immediate source-data changes, alias changes, index changes, bundle generation, or release actions.
...
| Field | Value |
| --- | --- |
| Candidate form: | `ndándayoro` |
| Canonical NFC form: | `[pending source-record normalization and provenance review]` |
| Canonical display form: | `ndándayoro` (same as candidate form) |
...
The approved candidate `ndándayoro` must be evaluated only as a complete
...
the current ndándayoro review and must not be answered by inference.
...
- Do not infer that a component inside `ndándayoro` is independently a
...
| `docs/reviews/phase7n2a_ndandadiya_lexical_review.md` | Companion review sheet |
