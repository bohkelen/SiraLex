/**
 * Phase 7N2A4F6 runtime smoke — retargeted in Phase 7N2A4F11.
 *
 ...
 *
 * After F8 promotion, this file verifies current catalog/runtime truth:
 * 7N2A is featured via VITE_FEATURED_BUNDLE_ID; 7J remains rollback-available.
 * Full featured-promotion install proofs live in phase7n2a4f8_*.test.ts.
 */
...
describe("Phase 7N2A4F6 runtime smoke (retargeted post-promotion)", () => {
  ...
  it("catalog lists both bundles; production env selects 7N2A as featured; 7J remains rollback", () => {
    ...
    expect(promoted!.url_base).toBe(`./${FEATURED_BUNDLE_ID}/`);
  ...
  it(
    "featured install activates 7N2A; search smoke passes; 7J remains installable as fallback",
    async () => {
      const featuredBundleId = readProductionFeaturedBundleId();
      const bundles = loadCatalogEntries();
  ...
  it(
    ...
    async () => {
      const tempManifestPath = join("/tmp", "phase7n2a4f11_f6_retarget_7l_manifest.json");
      writeFileSync(
        tempManifestPath,
        `${JSON.stringify(
          {
            ...
            case_count: 13,
            purpose: "Temporary F11 retargeted F6 7L runtime matrix (tracked matrices untouched).",
          },
          null,
          2,
        )}\n`,
