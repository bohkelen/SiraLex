/**
 * Phase 7N2A4F8 featured-promotion harness — retargeted in Phase 7N2B4G11.
 *
 * Historical F8 asserted 7N2A as featured via VITE_FEATURED_BUNDLE_ID. That
 * evidence remains archived in docs/reports/phase7n2a4f8_featured_promotion_report.md.
 *
 * After G11 promotion, this file tracks current featured/runtime truth:
 * 7N2B is featured; 7N2A remains prior/fallback; 7J remains older rollback.
 * Full G11 promotion proofs live in phase7n2b4g11_featured_promotion.test.ts.
 */
...
describe("Phase 7N2A4F8 featured promotion (G11 retarget)", () => {
  beforeEach(async () => {
  ...
  it("production env selects 7N2B as featured without relying on catalog sort", () => {
  ...
  it(
    "featured install path installs 7N2B and smoke search passes; 7N2A/7J remain installable",
    async () => {
      const featuredBundleId = readProductionFeaturedBundleId();
      const bundles = loadCatalogEntries();
  ...
  it(
    ...
    async () => {
      const tempManifestPath = join("/tmp", "phase7n2a4f8_g11_retarget_7l_manifest.json");
      writeFileSync(
        tempManifestPath,
        `${JSON.stringify(
          {
            ...
            case_count: 13,
            purpose: "Temporary F8 G11-retarget featured 7L runtime matrix (tracked matrices untouched).",
          },
          null,
          2,
        )}\n`,
